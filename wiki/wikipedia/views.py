...
def article_history(request,slug):
	...
	is form_is.valid():
		...
		if edit_form.is_valid():
			...
			edit.save()
			msg = "Articleupdated successfully"
			messages.success(request, msg, fail_silently = True)
			return render (request, 'wiki/article_form.html', {'form':form, 'edit_form':edit_form,' article':article})
		
def article_history(request, slug):
	article = get_object_or_404(Article, slug = slug)
	queryset = Edit.objects.filter(article_slug = slug)
	return render(request, 'wiki/edit_list.html', {'article':article, 'quesryset':queryset})
